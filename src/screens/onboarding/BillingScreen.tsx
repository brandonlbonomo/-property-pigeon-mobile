...
export function BillingScreen({ navigation }: any) {
  ...
  // Auto-advance if already subscribed or lifetime free
  useEffect(() => {
    if (isActive) navigation.replace('PortfolioSetup');
  }, [isActive]);

  // Fetch product prices from RevenueCat
  useEffect(() => {
    getProductPrices(accountType === 'cleaner' ? 'cleaner' : 'owner').then(setPricing);
  }, [accountType]);

  const planName = accountType === 'cleaner' ? 'Cleaner Pro' : 'Portfolio Pigeon Pro';

  // Dynamic price display
  const monthlyPrice = pricing.monthly?.priceString || (accountType === 'cleaner' ? '$7.99' : '$12.99');
  const yearlyPrice = pricing.yearly?.priceString || (accountType === 'cleaner' ? '$59.99' : '$99.99');
  const yearlyMonthly = pricing.yearly?.monthlyEquivalent || (accountType === 'cleaner' ? '$5.00' : '$8.33');
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      if (isCustomerEntitled(customerInfo)) {
        ...
        navigation.replace('PortfolioSetup');
      }
    } catch (e: any) {
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      <View style={styles.features}>
        {[
          ...
          ...(isSTR ? ['Calendar, inventory & cleaning management'] : []),
        ].map((feature, i) => (
      ...
      <View style={styles.buttons}>
        ...
        <TouchableOpacity style={styles.skipBtn} onPress={() => navigation.navigate('PortfolioSetup')} activeOpacity={0.8}>
