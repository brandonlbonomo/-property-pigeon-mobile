...
export function CreateAccountScreen({ navigation }: any) {
  ...
  // Basic format check (real validation via backend MX lookup)
  ...
  // Debounced email availability + domain validation check
  useEffect(() => {
    ...
    emailCheckTimer.current = setTimeout(async () => {
      try {
        ...
        if (!res.available) {
          setEmailTaken(true);
          setEmailError(res.error || 'Email not available');
        }
      } catch (err: any) {
        // Server returns error JSON for invalid domains
        if (err.serverError) {
          setEmailTaken(true);
          setEmailError(err.serverError);
        } else {
          setEmailTaken(false);
        }
      }
  ...
  const handleSubmit = async () => {
    if (!usernameValid) { Alert.alert('Invalid', usernameTaken ? 'Username is already taken' : !usernameCharsValid ? 'Username can only contain letters, numbers, dots, dashes, and underscores' : 'Username must be at least 3 characters'); return; }
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        {emailTaken && (
          <Text style={styles.errorText}>{emailError || 'This email is not available'}</Text>
        )}
